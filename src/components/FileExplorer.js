...
const FileExplorer = ({ tabId, tab, sshConfig, onClose, iconTheme = 'material', explorerFont = 'Segoe UI', explorerColorTheme = 'Light', setExplorerColorTheme, explorerFontSize = 15, isStandaloneTab = false }) => {
    ...
    return (
        <div
            className={isStandaloneTab ? "ssh-monitor-panel-standalone" : "ssh-monitor-overlay"}
            style={isStandaloneTab ? {
                width: '100%',
                height: '100%',
                display: 'flex',
            ...
            onClick={(e) => {
                if (!isStandaloneTab && e.target === e.currentTarget && !isResizing) onClose?.();
            }}
        >
            <ContextMenu
                model={contextItems}
            ...
            <div 
                className={isStandaloneTab ? `ssh-monitor-panel standalone-tab ${explorerColorTheme === 'Cyberpunk' ? 'cyberpunk-style' : ''}` : `ssh-monitor-panel ${hasMounted || isResizing ? 'no-animation' : ''} ${explorerColorTheme === 'Cyberpunk' ? 'cyberpunk-style' : ''}`} 
                style={{ 
                    display: 'flex', 
                    flexDirection: 'column',
