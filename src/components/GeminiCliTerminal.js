...
const GeminiCliTerminal = forwardRef(({
  ...
  useEffect(() => {
    ...
    }).catch((err) => console.error('[GeminiCliTerminal] Error cargando xterm:', err));
  ...
  const fitAndSyncSize = () => {
    try {
      ...
      window.electron?.ipcRenderer.send(`geminicli:resize:${tabId}`, {
  ...
  useEffect(() => {
    ...
    term.current = new Terminal({
      ...
      theme: {
        ...
        cursor: theme?.cursor || '#1a73e8',
        selection: theme?.selection || 'rgba(26, 115, 232, 0.3)',
        ...theme
      },
      cols: 120,
      rows: 30
    ...
    const resizeHandler = term.current.onResize(({ cols, rows }) => {
      window.electron?.ipcRenderer.send(`geminicli:resize:${tabId}`, { cols, rows });
    });

    const onDataUnsubscribe = window.electron?.ipcRenderer.on(`geminicli:data:${tabId}`, (data) => {
      writeBufferRef.current?.write(data);
    });

    const onReadyUnsubscribe = window.electron?.ipcRenderer.on(`geminicli:ready:${tabId}`, () => {
    ...
    const onErrorUnsubscribe = window.electron?.ipcRenderer.on(`geminicli:error:${tabId}`, (error) => {
      term.current?.writeln(`\x1b[31mGemini CLI Error: ${error}\x1b[0m`);
    });
    ...
    return () => {
      ...
      window.electron?.ipcRenderer.send(`geminicli:stop:${tabId}`);
  ...
  return (
    <div style={{
      ...
      <style>
        {`
          .geminicli-terminal-shell,
          .geminicli-terminal-shell .xterm,
          .geminicli-terminal-shell .xterm-viewport,
          .geminicli-terminal-shell .xterm-screen,
          .geminicli-terminal-shell .xterm-helpers {
      ...
      <div
        className="geminicli-terminal-shell"
        ref={terminalRef}
        style={{
          flex: 1,
          width: '100%',
...
export default GeminiCliTerminal;
