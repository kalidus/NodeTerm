...
const AntigravityCliTerminal = forwardRef(({
  ...
  useEffect(() => {
    ...
    }).catch((err) => console.error('[AntigravityCliTerminal] Error cargando xterm:', err));
  ...
  const fitAndSyncSize = () => {
    try {
      ...
      window.electron?.ipcRenderer.send(`antigravitycli:resize:${tabId}`, {
  ...
  useImperativeHandle(ref, () => ({
    ...
    paste: (text) => {
      if (text) {
        window.electron?.ipcRenderer.send(`antigravitycli:data:${tabId}`, text);
      }
    }
  }));
  ...
  useEffect(() => {
    ...
    term.current = new Terminal({
      ...
      theme: {
        ...
        cursor: theme?.cursor || '#4285f4',
        selection: theme?.selection || 'rgba(66, 133, 244, 0.3)',
        ...theme
      },
      cols: 120,
      rows: 30
    ...
    const resizeHandler = term.current.onResize(({ cols, rows }) => {
      window.electron?.ipcRenderer.send(`antigravitycli:resize:${tabId}`, { cols, rows });
    });

    const onDataUnsubscribe = window.electron?.ipcRenderer.on(`antigravitycli:data:${tabId}`, (data) => {
      writeBufferRef.current?.write(data);
    });

    const onReadyUnsubscribe = window.electron?.ipcRenderer.on(`antigravitycli:ready:${tabId}`, () => {
    ...
    const onErrorUnsubscribe = window.electron?.ipcRenderer.on(`antigravitycli:error:${tabId}`, (error) => {
      term.current?.writeln(`\x1b[31mAntigravity CLI Error: ${error}\x1b[0m`);
    });
    ...
    return () => {
      ...
      window.electron?.ipcRenderer.send(`antigravitycli:stop:${tabId}`);
  ...
  return (
    <div style={{
      ...
      <style>
        {`
          .antigravitycli-terminal-shell,
          .antigravitycli-terminal-shell .xterm,
          .antigravitycli-terminal-shell .xterm-viewport,
          .antigravitycli-terminal-shell .xterm-screen,
          .antigravitycli-terminal-shell .xterm-helpers {
      ...
      <div
        className="antigravitycli-terminal-shell"
        ref={terminalRef}
        style={{
          flex: 1,
          width: '100%',
...
export default AntigravityCliTerminal;
