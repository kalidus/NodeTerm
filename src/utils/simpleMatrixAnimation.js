...
export function initSimpleMatrixAnimation() {
  ...
  for (let i = 0; i < numColumns; i++) {
    createMatrixColumn(matrixContainer, i, columnWidth, matrixChars, speedMultiplier);
  }
}
...
function createMatrixColumn(container, index, columnWidth, chars, speedMultiplier) {
  ...
  createFallingChars(column, chars, index, speedMultiplier);
}

function createFallingChars(column, chars, columnIndex, speedMultiplier) {
  ...
  for (let i = 0; i < numChars; i++) {
    ...
    char.style.cssText = `
      ...
      font-size: 12px;
      font-weight: bold;
      text-shadow: 0 0 6px #00ff41, 0 0 12px #00ff41, 0 0 18px #00ff41;
