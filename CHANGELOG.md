...
- **Build Windows / IPC**: `sync-keys` movido a `src/main/utils` (siempre empaquetado); registro bootstrap de `appdata:*` y `get-system-stats` en `app.ready` antes de la ventana, con pasos aislados para que un fallo en Guacamole no deje la sidebar sin handlers.
