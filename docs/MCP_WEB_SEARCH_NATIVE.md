...
- `site_search(url, query, maxResults?)`: realiza una búsqueda `site:dominio` limitada al sitio indicado y devuelve enlaces relevantes.
- `fetch_page(url, maxLength?)`: descarga el HTML de una página (límites de tamaño/timeout).
- `extract_text(url, maxLength?)`: obtiene el HTML (renderizado si es necesario), detecta el cuerpo principal con Readability y devuelve resumen, Markdown y texto plano completo.
...
| `renderDelayMs` | Tiempo en milisegundos que el render espera tras `did-finish-load` antes de capturar el HTML (por defecto 1200 ms). |
...
5. Si `renderMode` está en `rendered` o la tool recibe `render: true`, se usa un `BrowserWindow` offscreen para obtener el HTML tras ejecutar JavaScript (similar a Puppeteer). Durante este proceso se bloquean ventanas emergentes y se espera un `renderDelayMs` antes de capturar el DOM.
6. Tras obtener el HTML (estático o renderizado), se ejecuta `Readability + Turndown` para extraer el contenido principal, generar Markdown y resumir. Se devuelven titulares, headings, enlaces y el texto completo (`data.markdown`, `data.rawText`).
7. `site_search` reutiliza DuckDuckGo con la consulta `site:dominio palabras` para ofrecer resultados centrados en un sitio concreto.
...
- `extract_text` depende de heurísticas + Readability; en páginas con marcado muy irregular puede ser necesario usar Puppeteer manualmente o inspeccionar `data.rawHtml`.
